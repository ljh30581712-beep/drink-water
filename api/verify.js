...
export default async function handler(req, res) {
  ...
  // Gemini가 일시적으로 과부하(503)일 때, 사용자에게 바로 오류를 보여주지 않고
  // 서버가 짧은 간격을 두고 최대 2회까지 자동으로 재시도해 체감 실패율을 낮춘다.
  ...
  try {
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      ...
      if (response.ok) break; // 성공하면 즉시 반복 종료

      // 503(과부하)만 재시도 대상. 429(한도초과)나 다른 오류는 바로 응답 처리.
      if (response.status !== 503 || attempt === MAX_ATTEMPTS) {
        lastErrText = await response.text();
        break;
      }
      lastErrText = await response.text();
      console.warn(`Gemini 503 재시도 ${attempt}/${MAX_ATTEMPTS - 1}`);
      await new Promise((r) => setTimeout(r, 1200)); // 1.2초 대기 후 재시도
    }
