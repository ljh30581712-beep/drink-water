...
Our team is made up of students at a semiconductor-focused Meister high school, so the environment we first pictured was the one most familiar to us: a semiconductor fab cleanroom. Semiconductor cleanrooms require wearing protective suits and passing through air showers, which raises the barrier to something as simple as drinking water far higher than in a typical office. During long stretches of focused work, thirst often goes unnoticed — and even when noticed, acting on it is inconvenient.

Through 6 real user interviews conducted at a semiconductor industry exhibition, however, we discovered two things. First, **workers in general industrial settings who don't wear cleanroom suits reported the exact same problem** — not drinking enough water because they don't have time or find it bothersome. Second, we found a practical constraint working the other way: semiconductor fab cleanrooms typically ban cameras outright, and touchscreens are hard to operate while wearing cleanroom gloves — meaning the "AI photo verification" approach we originally envisioned is actually difficult to apply in the very setting that inspired it.

Based on this, we broadened the scope of the problem beyond the semiconductor industry to **industrial workers in general who struggle to manage their own hydration timing**. In general manufacturing, logistics, and service settings without camera restrictions, the AI photo verification approach applies as-is; addressing camera-restricted special environments like semiconductor cleanrooms is treated as a separate future roadmap item (see Section 9).

The resulting chronic dehydration can lead to reduced concentration, fatigue buildup, and higher risk of safety incidents.
...
By naturally folding hydration into the "leave the work area → break area" route workers already take, the app reduces the psychological resistance tied to "wasting extra time" and improves on-site acceptance.

## 2. Real-World Usage Scenario (Full Cycle)

```
① Alarm rings → ② Wrap up current task → ③ Leave the work area → ④ Drink water at a water station → ⑤ AI verification in the app → (wait for next alarm, back to ①)
```
...
| **Minor safety** | This service is designed for adult industrial workers and does not assume use by minors. |
...
- **5-minute snooze**: For situations where immediate verification isn't possible (e.g. during hazardous work, or while moving between areas), a "Snooze 5 min" button pauses the alarm and re-triggers it in 5 minutes. **Allowed only once per alarm** — after that, the alarm keeps ringing until the user actually drinks water and completes AI verification.
...
- **Support for camera-restricted sites**: For special environments like semiconductor fab cleanrooms — where cameras are banned outright for security reasons, or where cleanroom gloves make touchscreen input difficult — we plan to explore alternative verification methods such as flow sensors at water stations or employee-badge tagging (a direct response to the constraint we confirmed through on-site interviews at a semiconductor industry exhibition).
...
## 10. Industries We Can Apply To

Water-Cycle's core principle — **"alarm at a set time → AI verifies the actual behavior"** — isn't limited to any single industry. It's directly applicable to **any setting where hydration management matters**.

| Setting | Example application |
|---|---|
| Semiconductor fab cleanrooms | For workers in protective suits. Camera-restricted zones require the alternative verification methods in Section 9 (sensors / badge tagging) |
...
| Warehouses / general manufacturing | The same structure applied to a wide range of production workers |
...
| Target scope redefinition | 6 on-site interviews at a semiconductor industry exhibition revealed that semiconductor fab cleanrooms themselves ban cameras outright and make touchscreen input difficult with cleanroom gloves — making AI photo verification hard to apply in the very setting we designed it for. At the same time, general industrial workers who don't wear cleanroom suits reported the same hydration problem | Expanded the target from semiconductor-only to industrial workplaces in general, wherever camera use isn't restricted. Camera-restricted special environments (e.g. semiconductor cleanrooms) were split off as a separate future roadmap item |

## Closing

**Water-Cycle** aims to establish healthy hydration habits across a wide range of industrial workplaces through one complete cycle: "alarm → wrap up → exit → drink → AI verification." By working within workers' existing movement patterns to maximize on-site acceptance, and by using AI-verified data to keep improving, we aim to grow this into a continuously evolving system. That's the core direction of this project.
